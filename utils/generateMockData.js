...
// Generate random answers and ratings
const generateRandomAnswer = (question) => {
  ...
  const ratings = [1, 2, 3, 4, 5]; // Ratings from 1 to 5
...
const insertMockData = async () => {
  for (let i = 1; i <= 10; i++) {
    for (const question of questions) {
      const { userAns, feedback, rating } = generateRandomAnswer(question);
      await db.insert(UserAnswer).values({
        mockIdRef: mockId,
        question: question,
        correctAns: "This is a placeholder for the correct answer.", // Adjust as necessary
        userAns: userAns,
        feedback: feedback,
        rating: rating.toString(), // Store rating as string
